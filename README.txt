...
1. Run the file install.sh. If not run by double-clicking, go to Terminal and take the directory. Then type sudo chmod 777 install.sh to
update permsissions. Then run the file by double clicking else type ./install.sh in terminal. 
...
install.sh. Thats all. Update is currently not available for AutoUpdater3.6 and it is not stable. 
